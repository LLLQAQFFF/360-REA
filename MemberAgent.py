...
class Member:
    ...
    def meeting(self, res_others):
        response = requestLLM('\n'.join(
            self.history) + "\nThe above is our previous conversation.\n" + "\n There are others' work\n" + res_others,
                              Meeting_prompt.format(total_task=self.total_task)).content
        # print(response)
        self.history.append(
    ...
    def complete_task(self, project, res_pre, feedback="", pre_exp=""):
        ...
        response = requestLLM(
            "\n".join(
                self.history[1:]) + "\nThe above is our previous conversation.\n" + self.history[0] + res_pre + feedback + pre_exp,
            Complete_prompt_).content
        self.history.append(
    ...
    def summarize_global_experience(self, project, global_exp):
        ...
        response = requestLLM(summarize_global_experience_prompt_, "").content
